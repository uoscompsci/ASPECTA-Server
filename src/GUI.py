class interface:
	...
	def addSurface(self, surface):
		...
		while(added != True):
			if(self.surfaces_lock == False):
				...
				index = len(self.surfaces)-1
				self.surfaces_lock = False
				added = True
		return index

	def getSurface(self, number):
	...
	def addDisplay(self, display):
		...
		index = 0
		while(added != True):
			if(self.displays_lock == False):
				self.displays_lock = True
				self.displays.append(surface)
				index = len(self.displays)-1
				self.displays_lock = False
				added = True
		return index

	def getDisplay(self, number):
		return self.displays[number]
	...
	def addCursor(self, cursor):
		...
		while(added != True):
			if(self.cursors_lock == False):
				...
				self.cursors.append(surface)
				index = len(self.cursors)-1
				self.cursors_lock = False
				added = True
		return index

	def getCursor(self, number):
		return self.cursors[number]
