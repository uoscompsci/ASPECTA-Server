...
class GUI:
    __slots__ = ['surfaces', 'surfaces_lock', 'displays', 'displays_lock', 'cursors', 'cursors_lock', 'lastcur',
                 'lastsur', 'lastwin', 'lastele' 'canvases', 'canvases_lock', 'elements', 'elements_lock',
                 'setup_surface_visible']
    ...
    def clearSurface(self, surfaceNo):
        ...
        canvasRemovalThread = threading.Thread(target=self.delayedCanvasRemoval,
                                               args=[canvases])  # Creates the display thread
    ...
    def moveCursor(self, cursorNo, xDist, yDist):
        ...
        if (self.findCursor(cursorNo) == 0):
            ...
            if (testLoc[0] > self.getSurfacePixelWidth(self.findCursor(cursorNo))):
                conn = self.testForConnection(self.findCursor(cursorNo) - 1, self.correctDirect["right"][
                    self.getSurfaceRotation(self.findCursor(cursorNo))])
                if (conn[1] == "None"):
                ...
                conn = self.testForConnection(self.findCursor(cursorNo) - 1, self.correctDirect["left"][
                    self.getSurfaceRotation(self.findCursor(cursorNo))])
                if (conn[1] == "None"):
                    finalLoc[0] = 0
                else:
            ...
            if (testLoc[1] > self.getSurfacePixelHeight(self.findCursor(cursorNo))):
                conn = self.testForConnection(self.findCursor(cursorNo) - 1, self.correctDirect["top"][
                    self.getSurfaceRotation(self.findCursor(cursorNo))])
                if (conn[1] == "None"):
                ...
                conn = self.testForConnection(self.findCursor(cursorNo) - 1, self.correctDirect["bottom"][
                    self.getSurfaceRotation(self.findCursor(cursorNo))])
                if (conn[1] == "None"):
                    finalLoc[1] = 0
                else:
    ...
    def newCircleWithID(self, owner, app, appno, ID, canvasNo, x, y, radius, coorSys, lineColor, lineWidth, fillColor,
                        sides):
        elementNo = self.newCircle(owner, app, appno, canvasNo, x, y, radius, coorSys, lineColor, lineWidth, fillColor,
                                   sides)
    ...
    def newRectangleWithID(self, owner, app, appno, ID, canvasNo, x, y, width, height, coorSys, lineColor, lineWidth,
                           fillColor):
        elementNo = self.newRectangle(owner, app, appno, canvasNo, x, y, width, height, coorSys, lineColor, lineWidth,
                                      fillColor)
        self.elements[set(elementNo)].setID(ID)
        return elementNo
    ...
    def removeElement(self, elementNo, canvas):
        ...
        canvas = str(canvas)
        removalThread = threading.Thread(target=self.delayedRemove,
                                         args=[canvas, elementNo])  # Creates the display thread
        removalThread.start()
